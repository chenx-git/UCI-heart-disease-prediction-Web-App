import os
os.chdir('/Users/chenxidong/Desktop/6004 project test/')
...
def main():
    st.title('🫀 Heart Diease prediction (SDSC6004 project)')
    st.sidebar.title('🫀 Heart Diease prediction')
    st.markdown('Four types of model to predict heart diease for UCI dataset')
    st.sidebar.markdown('Creater: DONG CHENXI')
    #simpply use the cache last time unlesss the input changed
    @st.cache(persist=True)
    def load_data():
        data=pd.read_csv('/Users/chenxidong/Desktop/6004 project test/heart.csv')
        data.columns = ['age', 
              'sex', 
              'cp', 
              'restbp', 
    ...
    st.sidebar.subheader('Choose method')
    classifier=st.sidebar.selectbox('Classifier',('Decision Tree Classifier','SVM','Logistic Regression','Random Forest'))
    
    if classifier == 'Decision Tree Classifier':
        st.sidebar.subheader('Model Parameters')
        #ccp_alpha
        ccp_alpha=st.sidebar.number_input('Minimal Cost-Complexity Pruning parameter',0.001,0.080,step=0.001,key='ccp_alpha')
    ...
    if classifier == 'Logistic Regression':
        st.sidebar.subheader('Model Parameters')
        C=st.sidebar.number_input('C (Regularization paramter)',0.010,10.00,step=0.010,key='C_LR')
